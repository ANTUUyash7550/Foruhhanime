...
async function openModal(anime){
  modalBody.innerHTML = `
    ...
    <div class="modal-watch" id="modalWatch"><p class="modal-watch-loading">Checking where this is streaming legally…</p></div>
  ...
  // Legal streaming platforms, straight from MyAnimeList's own data via Jikan.
  const watchEl = document.getElementById("modalWatch");
  if(anime.mal_id){
    try{
      const res = await jikan(`/anime/${anime.mal_id}/streaming`);
      const platforms = res.data || [];
      if(platforms.length){
        watchEl.innerHTML = `
          <div class="modal-watch-label">Watch legally on</div>
          <div class="modal-watch-links">
            ${platforms.map(p => `<a href="${p.url}" target="_blank" rel="noopener" class="watch-chip">${escapeHtml(p.name)}</a>`).join("")}
          </div>
          <p class="modal-watch-note">Sub/dub availability varies by platform and region — check the listing on the platform itself.</p>
        `;
      }else{
        watchEl.innerHTML = `<p class="modal-watch-note">No legal streaming platform listed on MyAnimeList for this title yet.</p>`;
      }
    }catch(e){
      watchEl.innerHTML = `<p class="modal-watch-note">Couldn't load streaming platforms right now.</p>`;
    }
  }else{
    watchEl.innerHTML = "";
  }
