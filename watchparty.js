(function(){
  ...
  window.openWatchParty = function(youtubeId, title){
    ...
    ensurePeerLib(ok => {
      if(!ok) statusEl.textContent = "Live connection library failed to load — check your internet connection and reopen this.";
    });
  };
  ...
  // Belt-and-braces: the × button, tapping the dark backdrop, and Escape all close it.
  ...
  // --- Peer library loading (primary CDN can be blocked on some networks — fall back) ---
  let peerLibState = "unknown"; // 'unknown' | 'loading' | 'ready' | 'failed'
  let peerLibWaiters = [];
  function ensurePeerLib(cb){
    if(typeof window.Peer !== "undefined"){ peerLibState = "ready"; cb(true); return; }
    if(peerLibState === "failed"){ cb(false); return; }
    peerLibWaiters.push(cb);
    if(peerLibState === "loading") return;
    peerLibState = "loading";
    const fallback = document.createElement("script");
    fallback.src = "https://cdn.jsdelivr.net/npm/peerjs@1.5.4/dist/peerjs.min.js";
    fallback.onload = () => {
      peerLibState = typeof window.Peer !== "undefined" ? "ready" : "failed";
      peerLibWaiters.forEach(f => f(peerLibState === "ready"));
      peerLibWaiters = [];
    };
    fallback.onerror = () => {
      peerLibState = "failed";
      peerLibWaiters.forEach(f => f(false));
      peerLibWaiters = [];
    };
  ...
  startBtn.addEventListener("click", async () => {
    ...
    ensurePeerLib(async ok => {
      if(!ok){ statusEl.textContent = "Couldn't load the connection library — check your internet and try again."; return; }
      try{
        isHost = true;
        await getMic();
        const code = genCode();
        ...
        statusEl.textContent = "Something went wrong starting the room — try again.";
      }
    });
  });
  ...
  joinBtn.addEventListener("click", async () => {
    ...
    ensurePeerLib(async ok => {
      ...
      try{
        ...
        statusEl.textContent = "Something went wrong joining the room — try again.";
      }
    });
  });
